...
#
# Strategy: pick ONE backend by availability, then block-or-fail on it.
# - flock(1) uses flock(2) locks; zsystem uses fcntl locks. These are different kernel
#   domains and do NOT mutually exclude — cascading between backends on contention would
#   grant two shells "the lock" at once. Availability is machine-uniform (every shell
#   picks the same backend); contention is not. Never select by contention outcome.
# - Lock is held ONLY during .zshrc sourcing (~0.5-2s per shell), not for the shell lifetime.
# - Blocking with timeout (default 30s). Workers queue FIFO-ish via kernel wait queue.
...
#
# Env overrides:
#   ZSHRC_LOCK_TIMEOUT=30          # seconds to wait before giving up (0 = infinite)
...
if [[ -z "${__ZSHRC_LOCK_TIMEOUT+x}" ]]; then
  typeset -g __ZSHRC_LOCK_TIMEOUT="${ZSHRC_LOCK_TIMEOUT:-30}"
fi
...
# Backend selection (once, by availability — see header for why never by contention)
if [[ -z "${__ZSHRC_LOCK_BACKEND+x}" ]]; then
  if command -v flock &>/dev/null; then
    typeset -g __ZSHRC_LOCK_BACKEND="flock"
  elif zmodload zsh/system 2>/dev/null; then
    typeset -g __ZSHRC_LOCK_BACKEND="zsystem"
  else
    typeset -g __ZSHRC_LOCK_BACKEND="mkdir"
  fi
fi
...
_zshrc_lock_acquire() {
  ...
  # Backend dispatch: exactly one backend runs (selected above by availability).
  # A backend that cannot acquire FAILS OPEN — it must not cascade: flock(2) and
  # fcntl locks live in different kernel domains, so a fallback lock would not
  ...
  case "${__ZSHRC_LOCK_BACKEND:-flock}" in
    flock)
      ...
      ;;
    zsystem)
      # zsystem flock needs the file to exist (ensured above) and opens its own FD via -f.
      # Module was loaded at backend selection; refresh in case it was unloaded since.
