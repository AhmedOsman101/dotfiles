...
1. **No Editors/Pagers**: `vim`, `nano`, `less`, `more`, `man` are BANNED.
2. **Force & Yes**: Preemptively supply "yes" or "force" flags when needed.
3. **Use Tools**: Prefer `Read`/`Write`/`Edit` tools over shell manipulation (`sed`, `echo`, `cat`).
4. **No Interactive Modes**: Never use `-i` or `-p` flags that require user input.
