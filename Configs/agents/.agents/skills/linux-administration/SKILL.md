...
- Package manager available (`apt` on Debian/Ubuntu, `pacman` or `paru` on Arch)
...
```bash
# Prefix all commands with `sudo -A`, check `SUDO_ASKPASS` variable, if not set; set it to `SUDO_ASKPASS=$(command -v askpass 2>/dev/null || get-askpass 2>/dev/null)`.
...
```bash
# Update all packages (official repos + AUR if using paru)
pacman -Syu
# or with paru (also updates AUR packages)
paru -Syu

# Search for a package (official repos)
pacman -Ss nginx

# Show package details (official repos)
pacman -Si nginx

# Install a package (official repos)
pacman -S nginx

# Install a specific version (Arch generally does not support this;
# you can use `downgrade` or install a specific package from the cache)
# Example: downgrade to a previous version
# pacman -U /var/cache/pacman/pkg/nginx-1.24.0-1-x86_64.pkg.tar.zst
# Or use `downgrade nginx`

# Remove a package (leaves dependencies)
pacman -R nginx
# Remove package + its unneeded dependencies
pacman -Rs nginx

# List installed packages, filter for nginx
pacman -Q | grep nginx
# Or search installed packages by name/description
pacman -Qs nginx

# Add an external repository (edit /etc/pacman.conf and enable e.g. [multilib])
# Then refresh the database:
pacman -Syy
# For AUR, install an AUR helper like paru or yay (manual setup required)

# View repository list (enabled repos are shown in /etc/pacman.conf)
rg "^\[.*\]" /etc/pacman.conf | rg -v "^#"

# Clean cached package files (keep only the latest versions)
pacman -Sc
# Clean ALL cached packages (remove everything from cache)
pacman -Scc
```
